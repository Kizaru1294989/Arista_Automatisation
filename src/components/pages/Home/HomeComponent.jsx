...
const labs = [
  ...
  { name: 'VXLAN L2 EVPN', image: EVPN, title: 'Lab Arista étape III : VXLAN L2 EVPN' },
];

export default function MediaControlCard({ lab, status, setLoadingDialog, formValue , setManuel , manuel }) {
  ...
  const renderLabImage = () => {
    ...
    return (
      <img src={currentLab.image} alt={`${currentLab.name} Lab Image`} style={{ width: '1700px', height: '700px' }} />
    );
  };
    ...
    <>
      ...
      <Grid sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%' }}>
        ...
        <CardContent sx={{ backgroundColor: 'black' }}>
            <Grid>
              <h1 style={{ display : 'flex' , color: 'white' , justifyContent: 'center', alignItems: 'center' }}>{labs[currentLabIndex].title}</h1>
            </Grid>
            <Grid>
              <h1 style={{ display : 'flex' ,  color: 'white', justifyContent: 'center', alignItems: 'center'  }}>Etape actuelle : {lab + " LAB " + status}</h1>
              {renderLabImage()}
            </Grid>
            <Grid sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
              ...
              {manuel &&    <Button style={{ display : 'flex' , justifyContent : 'center'}} variant="contained" onClick={() => handleInstructionsLab(labs[currentLabIndex].name)}>
           ...
           <ManualDialog lab={lab} instructions={instructions} setInstructions={setInstructions} open={open} setOpen={setOpen} currentLabIndex={currentLabIndex}/>
          </CardContent>
        </Card>
      </Grid>
    </>
